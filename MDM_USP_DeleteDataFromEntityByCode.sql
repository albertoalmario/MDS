...
/****** Object:  StoredProcedure [USPBR].[USP_DeleteDataFromEntityByCode]    Script Date: 6/23/2020 8:36:15 PM ******/
SET ANSI_NULLS ON
GO
...
-- Description:	SP que elimina un registro de una entidad
-- como parametro debe recibir el nombre del modelo, nombre de entidad, nombre de version y code
-- Nota: si no se quiere que se generen logs se debe cambiar el parematro @LogFlag = 1 por @LogFlag = 0
-- =============================================

CREATE PROCEDURE [USPBR].[USP_DeleteDataFromEntityByCode]
...
@CODE NVARCHAR(200)--Codigo Registro
...
BEGIN
	...
	SELECT @V_BATCHTAG = 'DEL-CODE-' + REPLACE(REPLACE(REPLACE(REPLACE(Convert(Nvarchar(50),Getdate(),121),'-',''),' ',''),':',''),'.','') 

	SET @V_QRY_SELECT = 'INSERT INTO [stg].[' + @V_ENTITY_STAGING_TABLE + '_Leaf] (BatchTag, ImportType, Code) SELECT ''' + @V_BATCHTAG + ''' as BatchTag, ''6'' AS ImportType, Code FROM mdm.' + @V_ENTITY_TABLE + ' WHERE Code = ''' + @CODE + ''''
	--PRINT @V_QRY_SELECT
	EXEC(@V_QRY_SELECT)
