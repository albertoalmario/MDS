USE [ReferenceData_ApplicationMDS_DEV]
GO

/****** Object:  StoredProcedure [USPBR].[USP_DeleteAllInactiveDataFromEntity]    Script Date: 6/23/2020 8:35:43 PM ******/
SET ANSI_NULLS ON
GO
...
-- Description:	SP que elimina toda la data inactiva que existe para una entidad
...
ALTER PROCEDURE [USPBR].[USP_DeleteAllInactiveDataFromEntity]
...
BEGIN
	...
	SET @V_QRY_SELECT = 'INSERT INTO [stg].[' + @V_ENTITY_STAGING_TABLE + '_Leaf] (BatchTag, ImportType, Code) SELECT ''' + @V_BATCHTAG + ''' as BatchTag, ''6'' AS ImportType, Code FROM mdm.' + @V_ENTITY_TABLE + ' WHERE Status_ID = 2'
	--PRINT @V_QRY_SELECT
	EXEC(@V_QRY_SELECT)
